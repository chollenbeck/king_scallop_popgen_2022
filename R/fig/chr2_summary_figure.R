...
ld_2 <- ld_all %>%
  filter(chrom1 == 2, chrom2 == 2)


ld_esp <- ld_2 %>%
  filter(pop == "ESP")

ggplot(ld_esp, aes(x=locus1, y=locus2, fill=r2)) + 
  geom_raster() + 
...
pantomime::get_allele_freqs(gen) %>%
  filter(locus %in% na_locs, allele == "0") %>%
  ggplot(aes(x = pop, y = freq)) +
  geom_col() +
  facet_wrap(~locus)

ld_esp %>%
  filter(locus1 == "HiC_scaffold_2_arrow_ctg1_46590731", locus2 == "HiC_scaffold_2_arrow_ctg1_46473090")
...
plot1 <- (chr2_fst + chr2_het + chr2_ies + chr2_rsb + plot_layout(ncol = 1) + 
            plot_annotation(tag_levels = 'A'))  / 
  (gg_2$NOR + gg_2$SCOT + gg_2$ESP + plot_layout(ncol = 3, guides = "collect"))


plot2 <- (chr2_fst + theme(legend.position = "none") + 
            chr2_het + 
            chr2_ies  + theme(legend.position = "none", 
...
ggsave(filename = "out/fig/chrom2_summary.png", plot = plot1, width = 15, height = 22,
       units = "cm")
